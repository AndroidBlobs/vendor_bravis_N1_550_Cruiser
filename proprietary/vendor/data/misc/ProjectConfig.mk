...
AUTO_ADD_GLOBAL_DEFINE_BY_VALUE = CUSTOM_HAL_LENS BOOT_LOGO MTK_AUDIO_BLOUD_CUSTOMPARAMETER_REV MTK_PLATFORM CUSTOM_KERNEL_LENS CUSTOM_KERNEL_MAIN_BACKUP_LENS MTK_BT_CHIP MTK_WLAN_CHIP CUSTOM_KERNEL_FLASHLIGHT CUSTOM_KERNEL_SUB_LENS MTK_FM_RX_AUDIO MTK_COMBO_CHIP MTK_GPS_CHIP CUSTOM_KERNEL_SUB_BACKUP_LENS CUSTOM_KERNEL_MAIN_LENS MTK_FM_TX_AUDIO MTK_FM_CHIP BOOT_LOGO
BOOT_LOGO = hd720m
BUILD_KERNEL = yes
BUILD_LK = yes
BUILD_MD32 = no
...
CUSTOM_MODEM = t939w_band1_band8_co_gps
...
############################### TEKSUN BEGIN#####################################
TEKSUN_DRIVE_COMMON_BOARD_DWS_NAME = HX_A6
XUNHU_PROJECT_MEMORY_TYPE=16.2die.ddr2.Discrete134ball
XUNHU_LPS_TEKHW_SUPPORT=yes
MTK_BUILD_VERNO=BRAVIS_N1_550_Cruiser_V3.17
BUILD_GMS=yes
XUNHU_QTY_SMART_WAKE_UP=no
XUNHU_FLIP_MUTE=no
XUNHU_AUTO_CLEAN_BACKGROUND_MEMORY=yes
XUNHU_QTY_GESTURE_SOMATOSENSORY=no
#功能:俄罗期项目拔号盘添加俄文字符
#使用方法:yes/no,默认no
XUNHU_DIALPAD_NEED_RU_STRING=yes
#小屏模式 
#使用方法:yes/no,默认no yes:添加小屏模式功能选项 no:移除小屏模式功能选项
XUNHU_SMALL_SCREEN_MODE=no
XUNHU_ADD_RAM_ROM_SHOW=yes
XUNHU_SETTING_RAM_VALUE=2
XUNHU_SETTING_ROM_VALUE=16
XUNHU_GESTURE_CONTROL_THREE_DELETE=yes
#统一设置型号、MTP、PTP、WIFI AP、USB、BT和磁盘名称
XUNHU_MODEL= N1_550_Cruiser
# 设置品牌名称，可以带空格
XUNHU_BRAND=BRAVIS
#设置设备名称，设备名称不能带空格
XUNHU_DEVICE= N1_550_Cruiser
#设置手机名称,手机名称最好不带空格，否则导致添加第三方应用出错.如goole play
XUNHU_NAME= N1_550_Cruiser
#设置手机制造商名称
XUNHU_MANUFACTURER=teksun
# 蓝牙名称，no为不设置，其他值为要设置的名称，可以带空格
XUNHU_BT= N1_550_Cruiser

#作者:trf1205
#功能:配置client id
#使用方法:填写需求的client id,默认no
XUNHU_CUSTOM_CLIENT_ID=android-teksun

#作者:trf1205
#功能:CTS配置
#使用方法:yes/no,默认no
XUNHU_COMMON_CTS_CONFIG=yes

#作者:trf1205
#功能:CTS fastpass+配置
#使用方法:yes/no,默认no
XUNHU_COMMON_CTS_FASTPASS_PLUS_CONFIG=yes


#功能:照相机前后摄像头默认预览大小(0 原始逻辑，进入照相机前后摄像头默认预览全部为全屏,1 进入照相机预览后置摄像头为全屏，前置摄像头为标准比例4:3,2 进入照相机预览后置摄像头为标准比例4:3，前置摄像头为全屏,3 进入照相机预览前后置摄像头全部为4:3)
#使用方法:0/1/2/3,默认0
XUNHU_DEFAULT_CAMERA_PREVIEW_SIZE=3
#作者:TRF261
#功能:是否编译老化测试AgingTest
#使用方法:yes/no,默认no
XUNHU_AGINGTEST_SUPPORT=yes

#功能:关机充电第一张图显示开机logo
#使用方法:yes/no,默认yes
XUNHU_LLQ_POWER_OFF_CHARGING_SHOW_LOGO=no
#功能:设置默认时区 #GMT+3 Jerusalem
XUNHU_DEFAULT_TIMEZONE=Asia/Jerusalem
#功能:设置默认语言
XUNHU_DEFAULT_LOCALE=ru-RU
# 功能：智能体感（原先的隔空手势)
# 使用方法：yes打开，no关闭
XUNHU_INTELLIGENT_SENSE=no
# yes打开，no关闭。如果支持的是单向光感，请设置XUNHU_ONE_WAY_SENSOR为yes，默认打开，否则应关闭，并关闭XUNHU_TWO_WAY_SENSOR
XUNHU_ONE_WAY_SENSOR=yes
# yes打开，no关闭。如果支持的是双向光感，请设置XUNHU_ONE_WAY_SENSOR为no，默认关闭，否则应打开，并关闭XUNHU_ONE_WAY_SENSOR
XUNHU_TWO_WAY_SENSOR=no
#进入相机界面，亮度默认最大，退出后恢复原有亮度
#使用方法:yes/no,默认no
XUNHU_CAMERA_BRIGHTNESS_MAX=yes
#对焦菜单
XUNHU_QTY_AF_SOUND_MENU=no
#功能:添加相机虚化功能
#使用方法:yes/no,默认no
XUNHU_BOKEH_SUPPORT=yes
#功能:添加前置摄像头虚化模式，必须先打开XUNHU_BOKEH_SUPPORT=yes
#使用方法:yes/no,默认no
XUNHU_BOKEH_FRONT=yes
# 功能：设置默认音量
# 使用方法：yes打开，no关闭 默认yes
XUNHU_ADJUST_CODE_FOR_DEFAULT_VALUE_SET=yes
# 功能：设置通话音量的默认值
# 使用方法：最大值：7，最小值：1，默认值为4
XUNHU_STREAM_VOICE_CALL=6
# 功能：设置系统音量的默认值
# 使用方法：最大值：15，最小值：0，默认值为15
XUNHU_STREAM_SYSTEM=14
# 功能：设置铃声音量的默认值
XUNHU_STREAM_RING=14
# 功能：设置多媒体音量的默认值
XUNHU_STREAM_MUSIC=14
# 功能：设置闹钟音量的默认值
XUNHU_STREAM_ALARM=14
# 功能：设置通知音量的默认值
XUNHU_STREAM_NOTIFICATION=14

#功能:使用YUV辅摄
#使用方法:yes/no,默认yes
#XUNHU_LPS_AUXILIARYCAMERA_SUPPORT=yes

#使用方法:yes/no,默认no, 下面四个宏需要同时打开，
#同时需要打开kernel-autoconfig　CONFIG_TRUSTKERNEL_TEE_SUPPORT=y
#对于1G一下的，MTK_TEE_DRAM_SIZE这个值为0x2400000，1G以上的这个值为0x4000000
MTK_TEE_DRAM_SIZE=0x2400000
MTK_TEE_SUPPORT = yes
TRUSTKERNEL_TEE_SUPPORT = yes
TRUSTKERNEL_TEE_RPMB_SUPPORT=yes
#瓶钵动态跳转地址支持，20180516后项目需打开此宏
TRUSTKERNEL_TEE_DYNAMIC_MACTH_SUPPORT=yes

#功能:去掉设置内数据保护
#使用方法:yes/no,默认no
XUNHU_NO_DATAPROTECTION=yes

#作者:TRF1222
#功能:添加查询RF频段暗码×#2369#
#使用方法:yes/no,默认no
XUNHU_QUERY_RF_BAND = no

TEKSUN_HARDWARE_CAMERA_SUPPORT = imx175_mipi_raw imx175_real_mipi_raw imx132_mipi_raw
#作者:TRF1222
#功能:屏蔽crash anr弹出框
#使用方法:yes/no,默认no
XUNHU_DELETE_ERROR_DIALOG=yes
############################### TEKSUN END#####################################
